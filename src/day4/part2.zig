...
fn count_rolls_in_map(map: []const u8, I: u16, J: u16) u16 {
    var n_rolls: u16 = 0;
    for (0..I) |i| {
        const i_u16 = @as(u16, @intCast(i));
        for (0..J) |j| {
...
fn mark_free_rolls(map: []u8, I: u16, J: u16) u16 {
    // replaces "@" rolls that are "free" with "x" and returns their total number
...
fn remove_free_rolls(map: []u8, I: u16, J: u16) u16 {
    // removes "x" rolls and returns their total number
    var n_rolls_removed: u16 = 0;
    for (0..I) |i| {
        const i_u16 = @as(u16, @intCast(i));
        for (0..J) |j| {
            const j_u16 = @as(u16, @intCast(j));
            if (map[ij_to_ndx(i_u16, j_u16, I, J)] == 'x') {
                map[ij_to_ndx(i_u16, j_u16, I, J)] = '.';
                n_rolls_removed += 1;
            }
        }
    }
    return n_rolls_removed;
}
...
pub fn main() !void {
    ...
    // count initial number of rolls
    const n_rolls_initial: u16 = count_rolls_in_map(map, I, J);
    var n_rolls_removed: u16 = 0;
    var n_removed_total: u16 = 0;
    var n_rounds: u16 = 0;
    while (n_rounds == 0 or n_rolls_removed > 0) {
        n_rolls_removed = mark_free_rolls(map, I, J);
        if (n_rolls_removed > 0) {
            std.debug.print("\nRemove {} roll{s} of paper:\n", .{
                n_rolls_removed, if (n_rolls_removed == 1) "" else "s"
            });
            debug_print_map(map, I, J);
        }
        n_rolls_removed = remove_free_rolls(map, I, J);
        n_rounds += 1;
        n_removed_total += n_rolls_removed;
    }

    // print and report final progress
    std.debug.print("From {} initial rolls, {} were removed in {} rounds for a total of {} remaining\n", .{
        n_rolls_initial, n_removed_total, n_rounds, n_rolls_initial - n_removed_total
    });
}
